...
class HomeView(EcomMixin,TemplateView):
    ...
    def get_context_data(self,**kwargs):
        ...
        paginator = Paginator(all_products, 3)#how 3 itmes per pages
...
class CheckoutView(EcomMixin,CreateView):
    ...
    success_url=reverse_lazy('RBB:home')
    ...
    def form_valid(self,form):
        ...
        if cart_id:
            ...
            order=form.save()
            if pm == "Esewa":
                return redirect(reverse("RBB:esewarequest") + "?o_id=" + str(order.id))
        else:
...
class AdminLoginView(FormView):
    ...
    form_class=CustomerLoginForm
    success_url=reverse_lazy('RBB:adminhome')
    def form_valid(self,form):
        uname=form.cleaned_data.get('username')
        pword=form.cleaned_data.get('password')
        usr=authenticate(username=uname,password=pword)
...
class AdminOrderStatusView(AdminRequiredMixin,View):
    def post(self,request,*args,**kwargs):
        ...
        order_obj.save()
        return redirect(reverse_lazy('RBB:adminorderdetail',kwargs={'pk':order_id}))
